...
本文档补充 `honeycomb-bee` 运行时在任务、assignment、worker、trigger 四类对象上的状态机约束，以及事件重放、重复提交和恢复时的幂等规则。
...
bee 运行时真正稳定的关键，不只是“有哪些状态”，而是“哪些迁移合法、重复输入怎么处理、恢复后怎么继续”。这三件事必须一起定义，系统行为才可预测。
